...
window.sliderPopup = (function (){
  ...
  zoomImg[2].addEventListener('click', function (evt) {
    evt.stopPropagation();
    sliderPopupWrapper.classList.toggle('slider-popup-wrapper--active');
    sliderPopupSlide.style.backgroundImage = 'url(' + document.querySelectorAll('.swiper-slide-active')[2].dataset.imgurl + ')';
    document.addEventListener('click', closeOnOuterClick);
    document.body.classList.add('no-scroll');
  });

  zoomImg[3].addEventListener('click', function (evt) {
    evt.stopPropagation();
    sliderPopupWrapper.classList.toggle('slider-popup-wrapper--active');
    sliderPopupSlide.style.backgroundImage = 'url(' + document.querySelectorAll('.swiper-slide-active')[3].dataset.imgurl + ')';
    document.addEventListener('click', closeOnOuterClick);
    document.body.classList.add('no-scroll');
  });
